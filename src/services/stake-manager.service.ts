...
export interface StakeSettings {
    ...
    isCustom: boolean;
    timestamp: number;
    aggressiveMode: boolean; // New aggressive mode setting
}

class StakeManagerService {
    private static instance: StakeManagerService;
    private readonly STORAGE_KEY = 'novaprime_stake_settings';
    ...
    private loadSettings(): StakeSettings {
        ...
        return {
            ...
            isCustom: false,
            timestamp: Date.now(),
            aggressiveMode: false, // Default to false
        };
    }
     ...
     */
    public getAggressiveMode(): boolean {
        return this.currentSettings.aggressiveMode;
    }

    /**
     * Update aggressive mode setting
     */
    public updateAggressiveMode(aggressiveMode: boolean): void {
        ...
        console.log('🔥 Aggressive mode updated:', {
            aggressiveMode: this.currentSettings.aggressiveMode,
        });
    }
     ...
     */
    public updateSettings(stake: number, martingale: number, aggressiveMode?: boolean): void {
        this.currentSettings = {
            stake: Math.max(0.01, stake), // Minimum stake of 0.01
            martingale: Math.max(1.0, martingale), // Minimum martingale of 1.0
            isCustom: true,
            timestamp: Date.now(),
            aggressiveMode: aggressiveMode !== undefined ? aggressiveMode : this.currentSettings.aggressiveMode,
        };
    ...
    public resetToDefaults(): void {
        this.currentSettings = {
            ...
            isCustom: false,
            timestamp: Date.now(),
            aggressiveMode: false, // Reset aggressive mode to false
        };
